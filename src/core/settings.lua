...
function dlt.parse(out,extra)
    if dlt.settings == nil then
        ...
        cmd:option('-seed',            torch.random(),                  'Seed (default is random).')
        ...
        for _,val in ipairs{'useCudnn','cudnnFastest','cudnnBenchmark',
                            'cudnnVerbose','dptFlatten','dptNccl','saveAll', 'useLocks',
                            'makeLogFile'} do
            dlt.settings[val] = dlt.help.boolify(dlt.settings[val])
        end 
        if extra then
            for _,val in ipairs(extra) do 
                if val[2] == 'false' or val[2] == 'true' then
                    dlt.settings[val[1]:sub(2,-1)] = dlt.help.boolify(dlt.settings[val[1]:sub(2,-1)])
                end
            end 
        end
    end
...
function dlt.configure(s)
    ...
    -- Make log file
    if s.makeLogFile then dlt.log:setFile(paths.concat(s.savePath,'log')) end 

    -- Check GPU
    local availGPU = dlt.have.cutorch and cutorch.getDeviceCount() or 0
    if s.nGPU > availGPU then
        dlt.log:warning(string.format('Available GPUs are %d, setting nGPU to %d', availGPU,availGPU))
        s.nGPU = availGPU
    end

    -- Check cudnn
    if s.useCudnn and not dlt.have.cudnn then
        dlt.log:warning('Cudnn could not be loaded make sure it is installed, switching cudnn use off.')
        s.useCudnn = false
    end
    ...
    if s.nGPU > 0  then 
        if s.nGPU > 1 and s.defGPU ~= 1 then
            dlt.log:warning('For multi-GPU use GPU 1 as default. Setting defGPU = 1.')
            s.defGPU = 1
        end
...
function dlt.reportExperiment(settings)
    ...
    if settings.nGPU > 0  then
        ...
        dlt.help.logAllGPUMemory(settings.nGPU,'yell')
        if settings.useCudnn then 
            dlt.log:yell(string.format('Using cudnn with: fastest = %s,'
                                    ..' benchmark = %s, verbose = %s',
                                    tostring(settings.cudnnFastest),tostring(settings.cudnnBenchmark),
                                    tostring(settings.cudnnVerbose) ) )
        else
            dlt.log:yell('Not using cudnn')
        end
        if settings.nGPU > 1 then
            dlt.log:yell(string.format('GPU parallelism using DataParallelTable with:'
                                        .. ' flattenParameters = %s, NCCL = %s',
                                        tostring(settings.dptFlatten),tostring(settings.dptNccl)))
        end
    else 
        dlt.log:yell('Running on CPU')
    end
    
    if settings.saveAll then dlt.log:yell('Will save all checkpoints') else dlt.log:yell('Will only save latest checkpoint') end
    dlt.log:yell('Save path: ' .. settings.savePath)
    dlt.log:yell('Mini-batch size: ' .. settings.batchSize)
    if settings.nThreads == 1 then dlt.log:yell('Will be loading data using 1 thread.') 
        elseif settings.nThreads > 1 then dlt.log:yell('Will be loading data using ' .. settings.nThreads .. ' threads.')
        else dlt.log:yell('Will not be using threads to load data.') end
    if settings.useLocks then dlt.log:yell('Locks are turned on.') else dlt.log:yell('Locks are turned off.') end
    dlt.log:endSection()
end

function dlt.writeSettingsToFile(s,fileName)
    local file = io.open(fileName, 'w+')
    for set,val in pairs(s) do file:write('-' .. set .. ' ' .. tostring(val) .. '\n') end
    file:close()
end