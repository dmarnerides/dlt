...
function D:__init(experimentFunction,extras)
    ...
    -- We do not store the actual experiment table but the function that returns it
    self.experiment = experimentFunction
    self.configuration = dlt.parse(nil,extras)
    -- First Make paths
    if not self.configuration.runRoot then dlt.log:error('Dispatcher: runRoot not given.') end
    if not self.configuration.experimentName then dlt.log:error('Dispatcher: experimentName not given.') end
    self.configuration.runRoot = dlt.help.checkHomePath(self.configuration.runRoot)
    dlt.help.checkMakeDir(self.configuration.runRoot,'runRoot')
    self.configuration.runPath = paths.concat(self.configuration.runRoot,self.configuration.experimentName)
    dlt.help.checkMakeDir(self.configuration.runPath,'runPath')
    self.configuration.savePath = paths.concat(self.configuration.runPath,self.configuration.savePath)
    dlt.help.checkMakeDir(self.configuration.savePath)

    -- Create lua script that runs dispatcher inside run directory
    dlt.writeSettingsToFile(self.configuration,paths.concat(self.configuration.runPath,'settings.txt'))
...
-- Do not use this, use D:run() if dispatcher was created and not loaded in script
function D:runLoadedDispatcher()
    -- Create settings
    -- We configure here because dispatcher may be created on a machine without a gpu
