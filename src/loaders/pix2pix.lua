...
-- s.name (cityscapes,maps,facades,edges2handbags,edges2shoes) 
--       defaults to cityscapes
function P:__init(s)    
    if dlt.help.inTable({'cityscapes','maps','facades',
                        'edges2handbags','edges2shoes'},s.name) then
        self.name = s.name
    ...
    if not paths.dirp(s.path) then 
        dlt.log:error('Could not find: ' .. s.path) 
    end
    ...
    -- paths
    self.path = {
        training = self.sets.training and paths.concat(s.path,'train') 
                                       or nil,
        validation = self.sets.validation and paths.concat(s.path,'val') 
                                           or nil,
        testing = self.sets.testing and paths.concat(s.path,'test') 
                                     or nil
    }
    
    if self.name == 'edges2handbags' or self.name == 'edges2shoes' then 
        self.format = '%d_AB.jpg'
    else 
        self.format = '%d.jpg' 
    end
end
...
function P:dataPoint(index,setName)
    local file = paths.concat(self.path[setName], 
                                string.format(self.format,index))
    return image.load(file,3,self.type)
end
