...
-- Note, parallelism was removed, but if using multiGPU then DPT will wrap the whole model

local function makeFeatures(w,h,inChannels,featureList,bn)
    local currentW,currentH = w,h
    local feat = nn.Sequential()
    feat:add(nn.SpatialConvolution(inChannels,featureList[1],11,11,4,4,2,2))       -- 224 -> 55
    if bn then feat:add(nn.SpatialBatchNormalization(featureList[1],1e-4)) end
    currentW,currentH = dlt.help.SpatialConvolutionSize(currentW,currentH,11,11,4,4,2,2)
    feat:add(nn.ReLU(true))
    feat:add(nn.SpatialMaxPooling(3,3,2,2))                   -- 55 ->  27
    currentW,currentH = dlt.help.SpatialMaxPoolingSize(currentW,currentH,3,3,2,2)
    feat:add(nn.SpatialConvolution(featureList[1],featureList[2],5,5,1,1,2,2))       --  27 -> 27
    if bn then feat:add(nn.SpatialBatchNormalization(featureList[2],1e-4)) end
    currentW,currentH = dlt.help.SpatialConvolutionSize(currentW,currentH,5,5,1,1,2,2)
    feat:add(nn.ReLU(true))
    feat:add(nn.SpatialMaxPooling(3,3,2,2))                   --  27 ->  13
    currentW,currentH = dlt.help.SpatialMaxPoolingSize(currentW,currentH,3,3,2,2)
    feat:add(nn.SpatialConvolution(featureList[2],featureList[3],3,3,1,1,1,1))      --  13 ->  13
    ...
    feat:add(nn.ReLU(true))
    feat:add(nn.SpatialConvolution(featureList[3],featureList[4],3,3,1,1,1,1))      --  13 ->  13
    if bn then feat:add(nn.SpatialBatchNormalization(featureList[4],1e-4)) end
    currentW,currentH = dlt.help.SpatialConvolutionSize(currentW,currentH,3,3,1,1,1,1)
    feat:add(nn.ReLU(true))
    feat:add(nn.SpatialConvolution(featureList[4],featureList[5],3,3,1,1,1,1))      --  13 ->  13
    if bn then feat:add(nn.SpatialBatchNormalization(featureList[5],1e-4)) end
    currentW,currentH = dlt.help.SpatialConvolutionSize(currentW,currentH,3,3,1,1,1,1)
    feat:add(nn.ReLU(true))
    feat:add(nn.SpatialMaxPooling(3,3,2,2))                   -- 13 -> 6
    currentW,currentH = dlt.help.SpatialMaxPoolingSize(currentW,currentH,3,3,2,2)
...
function dlt.models.alexnet(w,h,inChannels,nClasses)
    ...
    local fb1,currentW,currentH = makeFeatures(w,h,inChannels,{48,128,192,192,128},false) -- branch 1
    local fb2 = fb1:clone() -- branch 2
    for k,v in ipairs(fb2:findModules('nn.SpatialConvolution')) do v:reset() end -- reset branch 2's weights
...
function dlt.models.alexnet2(w,h,inChannels,nClasses)
    ...
    inChannels = inChannels or 3
    nClasses = nClasses or 1000
    local features,currentW,currentH = makeFeatures(w,h,inChannels,{64,192,384,256,256},true)
